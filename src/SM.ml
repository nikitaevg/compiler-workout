...
let rec eval env (callstack, stack, ((s, i, o) as config)) p =
    (*
    Printf.printf "%i, %i, %i\n" (List.length stack) (List.length i) (List.length o);
    let _ = match p with
            | x::_ -> Printf.printf "%s\n" (GT.transform(insn) (new @insn[show]) () x)
            | _ -> () in
    *)
    let eval_expr expr = match expr with
        | BINOP op -> (match stack with
            | (y::x::xs) -> (Language.Value.of_int (Language.Expr.str_to_op op
                                                   (Language.Value.to_int x)
                                                   (Language.Value.to_int y))
                                :: xs, config) 
            | _ -> failwith "Stack is empty on binop")
        | CONST x -> (Language.Value.of_int(x) :: stack, config)
        ...
        | ST name -> let (head, tail) = hd_tl stack "Stack is empty on store" in
                     ...
                     (tail, (new_state, i, o))
        | STA (name, id_count) ->
                let ((value :: ids), tail) = split (id_count + 1) stack in
                let new_state = Language.Stmt.update s name value ids in
                (tail, (new_state, i, o))
        | STRING str -> (Language.Value.of_string str :: stack, config)
    ...
    in match p with
        | x::xs -> (match x with 
            ...
            | CJMP (znz, label) -> let (head, tail) = hd_tl stack "Stack is empty on cjmp" in
                if cjmp_sat znz @@ Language.Value.to_int head
                then eval env (callstack, tail, config) (env#labeled label)
                else eval env (callstack, tail, config) xs
            | CALL (f, n, p) -> if env#is_label f then eval env ((xs, s)::callstack, stack, config) (env#labeled f)
                                else eval env (env#builtin (callstack, stack, config) f n (not p)) xs
  ...
  let (_, _, (_, _, o)) =
    eval
      (object
         ...
         method builtin (cstack, stack, (st, i, o)) f n is_proc =
           let f = match f.[0] with 'L' -> String.sub f 1 (String.length f - 1) | _ -> f in
           let args, stack' = split n stack in
           let (st, i, o, r) = Language.Builtin.eval (st, i, o, None) args f in
           let stack'' = if is_proc then stack' else let Some r = r in r::stack' in
...
let rec compile_expr e = match e with
    ...
    | Language.Expr.Array elems ->
            let compiled_elems = List.concat (List.map (compile_expr) (List.rev elems)) in
            compiled_elems @ [CALL ("$array", (List.length compiled_elems), true)]
    | Language.Expr.String str -> [STRING str]
    | Language.Expr.Elem (e, ind) -> compile_expr ind @ compile_expr e @ [CALL ("$elem", 2, true)]
    | Language.Expr.Length e -> compile_expr e @ [CALL ("$length", 1, true)]

let rec compile_impl lb p after_label = match p with
    | Language.Stmt.Assign (name, ids, expr) ->
            (match ids with
                | [] -> (compile_expr expr @ [ST name]), false, lb
                | ids -> let compiled_ids = List.fold_left
                            (fun p id -> p @ (compile_expr id)) [] (List.rev ids) in
                         compiled_ids @ compile_expr expr @
                         [STA (name, List.length ids)], false, lb)
